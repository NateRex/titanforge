...
constexpr const char* POST_PROCESS_VERTEX = R"(
	...
	void main()
	{
		// Generate an oversized full-screen triangle from the three vertex IDs.
		vec2 position = vec2(float((gl_VertexID << 1) & 2), float(gl_VertexID & 2));
		frag_TexCoord = position;

		// Convert the generated coordinates to clip space, covering the entire viewport.
...
constexpr const char* POST_PROCESS_FRAGMENT = R"(
	...
	void main()
	{
		// Sample the source image at the current screen position.
		vec4 source = texture(uMaterial.texture, frag_TexCoord);

		// Apply exposure, then calculate perceived brightness for saturation adjustment.
		vec3 color = source.rgb * uMaterial.exposure;
		float luminance = dot(color, vec3(0.2126, 0.7152, 0.0722));

		// Blend between grayscale and the exposed color, then adjust contrast around middle gray.
		color = mix(vec3(luminance), color, uMaterial.saturation);
		color = (color - 0.5) * uMaterial.contrast + 0.5;

		// Preserve the source alpha and apply the material's final color tint.
